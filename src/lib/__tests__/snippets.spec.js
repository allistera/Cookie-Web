...
import { getSlashSnippetCommands, normalizeSnippetName, sanitizeStoredSnippets } from '../snippets.js'
...
describe('snippet helpers', () => {
  ...
  it('keeps unique valid snippets and sanitizes their stored HTML', () => {
    expect(
      sanitizeStoredSnippets([
        { id: 'one', name: 'Hello World', html: '<p>Hello <strong>there</strong></p><script>alert(1)</script>' },
  ...
  it('turns snippets into distinguishable slash-menu commands', () => {
    expect(getSlashSnippetCommands([{ id: 'one', name: 'hello-world', html: '<p>Hello</p>' }])).toEqual([
      {
        id: 'snippet:one',
        type: 'snippet',
        title: 'hello-world',

...
describe('parseTimeLine', () => {
  ...
  it('does not let the range pattern swallow the end time into a single-time title', () => {
    // The single-time pattern alone would greedily match "10:00" / "11:00 -
    // Team sync"; the range pattern must be tried first.
    const result = parseTimeLine('10:00 - 11:00 - Team sync')
...
describe('syncDailyNoteEvents', () => {
  ...
  it('deletes events for lines removed from the document', async () => {
    const newBlocks = [oldBlocks[0]] // "removed" is gone; "keep" is unchanged
    const sql = queueSql(
      [], // DELETE for "removed"
  ...
  it('upserts a new or changed line with the resolved calendar', async () => {
    const sql = queueSql(
      [], // DELETE for "removed" (still absent from newBlocks)
      [{ id: 'cal-personal' }],
      [],
    )
  ...
  it('skips creating events when the user has no writable calendar', async () => {
    ...
    // resolveDefaultCalendarId returns null (empty result), so the loop
    // that would insert/upsert never runs.
    const sql = queueSql([])
    await syncDailyNoteEvents(sql, USER_ID, DOC_ID, '2026-08-14', [], newBlocks)

    expect(sql.statements).toHaveLength(1) // only the calendar lookup
  })
})

// Shared hybrid-retrieval legs for the documents search handler in
// documents.js. Mirrors retrieval.js's shape (keyword/recency/vector legs
// fused by rank-fusion.js) but is kept as a separate module since it targets
// a different table and a deliberately narrower filter vocabulary — documents
// have no sender/recipients/attachments/folders, only tags and a starred flag.
//
// The legs take a parsed spec { text, prefixQuery, filters } (see
// query-parse.js's parseDocumentSearchQuery). `text` is the free-text query,
// `prefixQuery` is an optional prefix tsquery for search-as-you-type, and
// `filters` holds the structured tag:/is:starred operators applied as extra
// SQL predicates across every leg.

import { normalizeDocumentTag } from '../../src/lib/documentTags.js'

// Full-text predicate: the free-text query OR, when present, the prefix
// query, so an in-progress last word still matches. Both are injection-safe:
...
// generated column already boosts title over body via ts_rank's default
// weights.
